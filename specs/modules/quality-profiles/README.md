...
2. **Quality size definitions** — per-bin `min` / `preferred` / `max` file sizes, runtime-scaled (Sonarr/Radarr's `QualityDefinition`). This is the canonical reason movie and series profiles are separate configs: a sane movie size band rejects every episode and vice-versa. One band feeds three places — a hard gate (outside `[min, max]` → reject), a soft score (closeness to `preferred` → bump), and the tie-break (size nearest `preferred`). Profile-owned, per-domain data.
