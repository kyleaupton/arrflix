...
5. **Soft scoring** — preferences that contribute to a release's score within its quality bin (preferred release groups, codec preferences, audio, HDR variants, proper/repack). **Iteration 1 ships a fixed, closed set of structured scorers — not a custom-format DSL** (see [open questions](#open-questions)). See same section.
...
1. **Custom format syntax. (v1 scope resolved.)** Iteration 1 ships a **fixed, closed set of structured scorers** — preferred/blocked release groups, codec preference, HDR/audio variant, proper/repack — and **no custom-format DSL**. This keeps the soft-score data model concrete (named scorers with weights) rather than an open-ended rule store. The general syntax question (Sonarr-style regex vs. higher-level structured rules vs. both) is deferred to the iteration that builds the advanced "Custom formats" UX tier; lean remains structured-first with a regex escape hatch, structured rules compiling to regex under the hood.
