...
func NewRegistry() *Registry {
	fields := make(map[string]model.ContextFieldInfo)
...
func (r *Registry) Field(path string) (model.ContextFieldInfo, bool) {
	f, ok := r.fields[path]
	return f, ok
}

// HasPhase reports whether the tree references any field of the given phase.
// Consumers use it to detect post-download conditions (the import-time
// re-gate) and to drive phase badges in the rule editor.
